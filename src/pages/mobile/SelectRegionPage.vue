...
onMounted(async () => {
  ...
  try {
    ...
    takenRegionIds.value = (data.players || [])
      .filter((p) => p.region_id && p.id !== session.playerId)
      .map((p) => p.region_id)
  } catch (e) {
...
<template>
  <MobileLayout>
    ...
    <p class="text-muted mb-4">เลือกพื้นที่ปลูกของคุณในประเทศนี้</p>
    ...
    <div class="card card-farm p-3 mb-3">
      <RegionSelector
        ...
        :taken-region-ids="takenRegionIds"
      />
