...
export function validateCrop(playerId, { crop_name }) {
  return api.post(`/players/${playerId}/validate-crop`, { crop_name }).then(unwrap)
}
...
export function answerBonusQuiz(playerId, answer) {
  return api.post(`/players/${playerId}/bonus-quiz/answer`, { answer }).then(unwrap)
}

export function pingBonusQuizActivity(playerId) {
  return api.post(`/players/${playerId}/bonus-quiz/activity`).then(unwrap)
}

export function startPlanAdjustment(playerId) {
  return api.post(`/players/${playerId}/plan-adjustment/start`).then(unwrap)
}

export function finishPlanAdjustment(playerId) {
  return api.post(`/players/${playerId}/plan-adjustment/finish`).then(unwrap)
}

export function cancelPlanAdjustment(playerId) {
  return api.post(`/players/${playerId}/plan-adjustment/cancel`).then(unwrap)
}
