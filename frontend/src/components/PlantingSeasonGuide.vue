...
function suitabilityClass(mult) {
  const pct = (mult || 0) * 100
        ...
        <table class="table table-sm table-bordered mb-0 small">
          ...
          <tbody>
            <tr v-for="c in guide.crops" :key="c.id">
              ...
              <td>{{ Math.round((c.base_coin || 100) * (c.coin_multiplier || 1)) }}</td>
              <td>
                <span :class="suitabilityClass(c.coin_multiplier)">
                  {{ Math.round((c.coin_multiplier || 1) * 100) }}%
                </span>
              </td>
